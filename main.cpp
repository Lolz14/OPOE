...
int main() {
    ...
    auto hull_model = std::make_shared<HullWhiteModelSDE<R>>(0.04, 2.0, 0.20, 0.3, -0.7, x0);
    ...
    [hull_model](R t0, R ttm, int num_steps, int num_paths, const std::optional<SDEMatrix>& dW_opt) {
        return EulerMaruyamaSolver<HullWhiteModelSDE<R>, R> (*hull_model).solve(t0, ttm, num_steps, num_paths, dW_opt);});

    std::cout << "Option Price: " << ope_pricer.price() << std::endl;
